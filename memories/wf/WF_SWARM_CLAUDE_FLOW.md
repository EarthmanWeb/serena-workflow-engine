# WF_SWARM_CLAUDE_FLOW - Claude-Flow V3 Swarm Methodology
...
**⛔ WRONG prefixes (never use):** ~~`mcp__plugin_claude-flow_claude-flow__`~~
...
```javascript
// 1. Load needed claude-flow tools (ONE ToolSearch call)
ToolSearch({ query: "+claude-flow swarm agent task" })
// If you need memory tools too:
ToolSearch({ query: "select:mcp__claude-flow__memory_store,mcp__claude-flow__memory_retrieve" })
...
mcp__claude-flow__agent_spawn({ agentType: "researcher", agentId: "researcher-1" })
mcp__claude-flow__agent_spawn({ agentType: "coder", agentId: "coder-1" })
mcp__claude-flow__agent_spawn({ agentType: "analyst", agentId: "analyst-1" })
...
// task_dependencies({ taskId: "task-2", dependsOn: ["task-1"] })
```
...
// One Agent per claude-flow agent — MCP coordinates, Agent tool executes
```
...
mcp__claude-flow__task_status({ taskId: "task-id" })
mcp__claude-flow__swarm_status({})  // NO verbose flag

// 8. After Task agents complete, update task status
mcp__claude-flow__task_complete({ taskId: "task-id" })
...
mcp__claude-flow__memory_retrieve({ key: "results-summary" })
```
...
| `swarm_status` | `mcp__claude-flow__swarm_status` | Check health (NO verbose flag) |
...
| `task_create` | `mcp__claude-flow__task_create` | Register task with agent assignment |
| `task_status` | `mcp__claude-flow__task_status` | Check task progress |
| `task_complete` | `mcp__claude-flow__task_complete` | Mark task done |
| `memory_store` | `mcp__claude-flow__memory_store` | Persist state |
| `memory_retrieve` | `mcp__claude-flow__memory_retrieve` | Recall state |
...
- **Fire-and-forget MCP tasks** — collect results via Agent tool output, not `task_results` MCP call
