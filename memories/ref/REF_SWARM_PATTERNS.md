---
name: REF_SWARM_PATTERNS
description: Multi-agent swarm coordination reference — tool selection, context budget, execution paths, agent-prompt bypass template.
metadata:
  type: reference
---
...
Multi-agent patterns for parallel work and complex task coordination.

## DEFAULT: Claude Code Agent Tool

Use the Claude Code `Agent` tool as the PRIMARY mechanism for ALL parallel work involving file access. Use Ruflo/DAA/Hive-Mind ONLY for cognitive-load tasks (reasoning, planning, consensus) with NO file-system access.
...
Launch all parallel agents in ONE message. Prefix every prompt with the swarm bypass block (see Mandatory Agent Prompt Template). Results arrive via background task notifications.

### Escalate to Ruflo ONLY when ALL true
...
3. Coordination value justifies ~10-15 MCP-call overhead.

If any agent needs file access, use the Claude Code `Agent` tool directly. `agent_execute` CANNOT read files.

## Verified MCP Tool Prefix (2026-05-06)

All swarm systems use prefix `mcp__ruflo__`.

| System | Example Tool |
| ------ | ------------ |
| Ruflo (unified) | `mcp__ruflo__swarm_init` |
| Hive-Mind | `mcp__ruflo__hive-mind_init` |
| DAA | `mcp__ruflo__daa_agent_create` |
| Coordination | `mcp__ruflo__coordination_orchestrate` |

## Context Budget Rules

Swarm sessions fail from context overload: every MCP call adds request+response tokens, every memory read adds content, the state machine compounds it.

| Budget Item | Token Estimate | Rule |
| ----------- | -------------- | ---- |
| Workflow init (WF_INIT → WF_CLASSIFY) | ~15-25K | Unavoidable baseline |
| Each memory read | ~1-5K | Read only what you NEED |
| Each MCP tool schema (ToolSearch) | ~500-1K | Load only tools you'll USE |
| Each MCP tool response | ~200-2K | NEVER use verbose/detailed flags |
| Task agent prompt | ~2-5K | Keep agent prompts focused |

Set BEFORE launching claude:

```bash
export MAX_MCP_OUTPUT_TOKENS=5000    # Cap MCP responses (default 25K overflows)
...
3. NEVER call `memory_stats` — it scans up to 100K entries internally.
4. Limit `memory_list` to `limit: 5` (default is 50).
5. Skip `memory_search` unless absolutely needed — it returns full values.
6. Use Task agents for ALL file work — they have separate context windows.
7. Keep coordinator context lean — it orchestrates only, never reads code.
8. Batch MCP calls — init+spawn+task in ONE message, not separate turns.
9. Skip `swarm_status`/`task_status` unless needed — each adds ~1-2K tokens.
10. Fire-and-forget MCP tasks — use `TaskOutput` on Task agents, NOT the `task_results` MCP call.

## Tool Selection Hierarchy

Default to the Claude Code `Agent` tool. Use Ruflo ONLY when explicitly requested or when cognitive-only tasks need it.
...
Use MCP tools directly (in-memory coordination only). NEVER run `npx ruflo init` — it modifies repo files. Use `mcp__ruflo__swarm_init({ topology: "mesh" })` instead.

## Swarm Subsystems

### 1. Ruflo Swarm (primary for orchestration)

Prefix `mcp__ruflo__`. Load ONLY tools you need:
...
| `daa_agent_create` | Metadata record with cognitive-pattern label | NO — bookkeeping only | You need cross-iteration tracking (DAA) only |

NEVER create `daa_agent_create` agents and expect them to execute — they are metadata ONLY. ALWAYS use `agent_spawn` → `agent_execute` for Ruflo-native execution.

Three execution paths, ordered by preference:

| Path | Tools | File Access? | Use When |
|------|-------|-------------|----------|
| Claude Code Agent (DEFAULT) | Claude Code `Agent` tool directly | Yes (full tool access) | Any task needing file access — codebase analysis, edits, grep, symbol lookup. No Ruflo needed. |
...
| Hybrid | `agent_spawn` (tracking) + Claude Code `Agent` tool (execution) | Yes (full tool access) | Need BOTH file access AND Ruflo cross-iteration tracking |

Ruflo-native workflow (reasoning tasks):
...
// Execute on ALL agents in ONE message (parallel)
...
// Results return directly — no TaskOutput needed
```

Hybrid workflow (codebase file access):
...
Prefix `mcp__ruflo__coordination_*`. Two SEPARATE agent systems — do NOT mix:
...
DAA is a metadata/tracking layer, NOT an execution engine. `daa_workflow_execute` flips a status flag and returns empty arrays. `daa_agent_create` creates metadata records ONLY — they cannot execute. For actual execution use `agent_spawn` + `agent_execute` (Ruflo-native) OR `agent_spawn` + Claude Code `Agent` tool (hybrid for file access). Use DAA ONLY for multi-iteration workflows needing structured cross-iteration state; for single-pass parallel work use Ruflo swarm orchestration.

DAA + Ruflo-native execution:
...
// ALSO spawn executable Ruflo agents (these actually run)
...
DAA + Hybrid execution (file access):
...
Agent subagents MUST be explicitly instructed to use hive-mind tools. Without explicit `ToolSearch` + `hive-mind_memory` instructions in the agent prompt, agents run as plain parallel workers, NOT hive-mind participants. Use the `WF_SWARM_HIVE_MIND` Phase 4 agent-prompt template.
...
4. Use `agent_execute` for reasoning/planning tasks — results return directly, no Task/Agent overhead.
5. Use the Claude Code `Agent` tool ONLY when file-system access is needed (Grep, Read, Glob, etc.).
6. Keep coordinator lean — orchestrate only.
7. Batch all ToolSearch loads into ONE call.

DON'T:

1. Do NOT load tools you won't use.
2. Do NOT use verbose/detailed flags on MCP calls.
3. Do NOT call `memory_stats` — it scans all entries.
4. Do NOT read memory docs during swarm execution — load them BEFORE.
5. Do NOT block on the first agent before spawning others.
6. Do NOT read files directly in the coordinator — agents do that.
7. Do NOT use `SendMessage` — it does NOT exist. The Agent tool output suggests it, but the tool is unavailable. Agents are fire-and-forget.

## Agent Communication Constraints (2026-05-22)

Standard mode: `SendMessage` does NOT exist. The Agent tool's completion output suggests it, but the tool is unavailable in standard sessions.

Experimental Agent Teams mode: `SendMessage` IS available when `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` is set — enables peer-to-peer messaging between teammates. Agent Teams are experimental: no session resumption, ~3-7x token cost, no file isolation.
...
- Treat agents as fire-and-forget workers.
...
When using the Claude Code `Agent` tool for swarm agents, EVERY prompt MUST start with:
...
Without this block, Claude Code Agents re-run the full init sequence (WF_INIT → CLAUDE_OBLIGATIONS → WF_CLASSIFY), consuming their entire context window on workflow overhead instead of the assigned task.

## Known Issues

| Issue | Impact | Mitigation |
| ----- | ------ | ---------- |
| Pretty-printed JSON responses | 2x response size | Cannot control — keep calls minimal |
...
| Agent tool claims SendMessage works | Misleading output on agent completion | Ignore the suggestion — tool is unavailable |
| Spawned agents re-run WF_INIT | Agent wastes full context on init | Include swarm bypass block in EVERY Agent tool prompt (see template above) |
...
| Coordinator uses Agent tool ignoring spawned Ruflo agents | Ruflo agents never execute, no tracking | Use `agent_execute` on spawned agents; use Agent tool only for hybrid (file access) path |
