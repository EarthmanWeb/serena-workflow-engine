#!/bin/bash
# Hook: claude-flow-post-bash.sh
# Purpose: Conditionally invoke claude-flow post-command hook when operating as claude-flow agent
...
if [ "$IS_CLAUDE_FLOW" = "true" ] && [ -n "$COMMAND" ]; then
    ...
    if command -v npx &> /dev/null; then
        # Run claude-flow post-command hook (non-blocking, capture errors)
        npx claude-flow@alpha hooks post-command \
            --command "$COMMAND" \
            --track-metrics true \
            --store-results true 2>&1 || true

        CF_EXIT=$?
        if [ $CF_EXIT -ne 0 ]; then
            echo "claude-flow post-command: warning (exit $CF_EXIT)" >&2
        fi
    else
        echo "claude-flow: npx not available, skipping post-command hook" >&2
    fi
fi
