...
> **Plugin template.** Copy to project Serena memories and customize
> placeholders.
...
The test gate hook (`swe_pre_bash_test_gate.py`) **blocks**
`npx playwright test` commands until this memory has been read in the current
session.

**How it works:**

1. `swe_pre_bash_test_gate.py` intercepts Bash commands matching
   `npx playwright test`
...
**Sentinel naming:** `.test_feature_{session_id}` — session-scoped, no
...
To gate additional commands (e.g., `npm run test`), add patterns to the list in
the pre-hook.
...
1. **Replace placeholders** (`{test_root}`, `{base_url}`, etc.) with actual
   values
